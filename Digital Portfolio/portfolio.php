<?php
// index.php - Dynamic Portfolio Page
session_start();
require 'db.php';
...
// Function to get files for a category, respecting permissions for visitors
function getFiles($pdo, $category_id, $user_role, $user_id) {
    if ($user_role === 'Administrator') {
        $stmt = $pdo->prepare("SELECT * FROM files WHERE category_id = ? AND status = 'active'");
        $stmt->execute([$category_id]);
    } else {
        // For visitors, only show files they have permission for
        $stmt = $pdo->prepare("
            SELECT f.* FROM files f
            INNER JOIN permissions p ON f.id = p.file_id
            WHERE f.category_id = ? AND f.status = 'active' AND p.user_id = ?
        ");
...
<head>
    ...
    <!-- Configure Tailwind to use CSS Variables and enable class-based theme switching -->
    <script>
        tailwind.config = {
            // We use the 'light' class on the HTML tag to denote light mode
            darkMode: 'class', 
            theme: {
                extend: {
                    ...
                    colors: {
                        ...
                        // Specific colors needed for non-swapping elements
                        'success-bg': '#15803d', 
                        'success-text': '#d1fae5',
                        'error-bg': '#b91c1c',
                        'error-text': '#fee2e2',
...
<body class="antialiased">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            ...
            <nav class="hidden md:flex space-x-8 items-center">
                ...
                <button id="theme-toggle-desktop" aria-label="Toggle theme" class="p-2 rounded-full text-accent hover:bg-accent/10 transition duration-300 focus:outline-none focus:ring-2 focus:ring-accent">
                    <!-- Icon placeholder will be set by JS on load -->
                    <svg id="theme-icon-desktop" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"></path><path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.12-1.36a6.997 6.997 0 01-5.88 5.88A9 9 0 0012 3z"></path></svg>
                </button>
                <!-- Logout Link -->
                <a href="logout.php" class="text-text-muted hover:text-accent transition duration-300 font-medium">Logout</a>
            </nav>

            <!-- Mobile Menu and Toggle Button Group -->
            <div class="flex items-center space-x-4 md:hidden">
                ...
                <button id="theme-toggle-mobile" aria-label="Toggle theme" class="p-2 rounded-full text-accent hover:bg-accent/10 transition duration-300 focus:outline-none focus:ring-2 focus:ring-accent">
                    <svg id="theme-icon-mobile" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"></path><path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.12-1.36a6.997 6.997 0 01-5.88 5.88A9 9 0 0012 3z"></path></svg>
                </button>
                 <!-- Mobile Menu Button (Hamburger) -->
                <button id="mobile-menu-button" class="p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-accent">
                    <svg class="w-6 h-6 text-text-default" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
                </button>
            </div>
        </div>
    ...
    <main>
        ...
        <section id="about" class="py-20 px-4 sm:px-6 lg:px-8 bg-primary transition-colors duration-300">
            <div class="container mx-auto max-w-5xl flex flex-col lg:flex-row items-center gap-12">
                ...
                <div class="flex-shrink-0 w-full lg:w-1/3">
                    <!-- Note: The image URL provided by the user is a placeholder in this environment. -->
                    <img src="img/profile pic.jpeg" alt="Student Profile Picture" 
        ...
        <!-- Academic Portfolio Section (Collapsible Structure) -->
                            ...
                            <div class="space-y-6 border-l-4 border-accent pl-6">
                                ...
                                <div class="bg-secondary/50 rounded-lg shadow-md border border-accent/50">
                                    ...
                                    <div id="semester-1" class="semester-content hidden p-4 pt-0 space-y-4">
                                        ...
                                        <!-- Period 1: Web Development Basics (8 Weeks) -->
                                        <div class="bg-primary p-4 rounded-lg shadow-inner border border-border-color transition-colors duration-300">
                                            ...
                                            <div id="period-1" class="period-content hidden mt-3 space-y-4">
                                                <p class="text-sm text-text-muted mb-4 border-b border-border-color pb-2">
                                                    **Project Focus:** Full design and initial development of the **Sunny Socks** e-commerce website. Weeks 1-5 link to individual assignments, while Weeks 6-8 link to the shared **Team Project Repository** on GitHub.
                                                </p>
                                                <ul class="list-none space-y-2 text-text-default">
                                                    <?php
                                                    // Fetch category ID for Period 1 Web Dev
                                                    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
                                                    $stmt->execute(['Period 1: Web Development Basics (HTML, CSS, PHP, Figma)']);
                                                    $cat = $stmt->fetch();
                                                    $period1_id = $cat ? $cat['id'] : null;
                                                    $period1_files = $period1_id ? getFiles($pdo, $period1_id, $user_role, $user_id) : [];
                                                    if (empty($period1_files)): ?>
                                                    <li class="p-2 text-text-muted italic">No additional files uploaded yet.</li>
                                                    <?php else: ?>
                                                    <?php foreach ($period1_files as $file): ?>
                                        ...
                                        <!-- Period 1: Professional Skills & Documentation (Separate Module) -->
                                        <div class="bg-primary p-4 rounded-lg shadow-inner border border-accent/70 transition-colors duration-300">
                                            ...
                                            <div id="period-1-prof-skills" class="period-content hidden mt-3 space-y-2">
                                                <p class="text-sm text-text-muted mb-2 font-semibold border-b border-border-color pb-2">
                                                    Key non-technical documentation showcasing teamwork and project planning for the web development module.
                                                </p>
                                                <ul class="list-none space-y-2 text-text-default pt-2">
                                                    <?php
                                                    // Fetch category ID for Period 1 Prof Skills
                                                    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
                                                    $stmt->execute(['Period 1 Professional Skills & Documentation']);
                                                    $cat = $stmt->fetch();
                                                    $period1_prof_id = $cat ? $cat['id'] : null;
                                                    $period1_prof_files = $period1_prof_id ? getFiles($pdo, $period1_prof_id, $user_role, $user_id) : [];
                                                    if (empty($period1_prof_files)): ?>
                                                    <li class="p-2 text-text-muted italic">No files uploaded yet.</li>
                                                    <?php else: ?>
                                                    <?php foreach ($period1_prof_files as $file): ?>
                                                    <li class="p-2 border-b border-border-color flex justify-between items-center hover:bg-secondary rounded-sm transition-colors duration-300">
                                                        <span class="font-medium"><?php echo htmlspecialchars($file['title']); ?></span> 
                                                        <span class="text-sm text-text-muted hidden md:block"><?php echo htmlspecialchars($file['description']); ?></span>
                                                        <a href="<?php echo htmlspecialchars($file['file_path']); ?>" target="_blank" class="text-accent hover:text-accent-dark text-sm font-medium" data-filetype="<?php echo pathinfo($file['file_path'], PATHINFO_EXTENSION); ?>">(View Artifact)</a>
                                        ...
                                        <!-- Period 2: Database Management (8 Weeks) -->
                                        <div class="bg-primary p-4 rounded-lg shadow-inner border border-border-color transition-colors duration-300">
                                            ...
                                            <div id="period-2" class="period-content hidden mt-3 space-y-4">
                                                <p class="text-sm text-text-muted mb-4 border-b border-border-color pb-2">
                                                    **Project Focus:** Mastering **Relational Database Management** using **MySQL**, administration via **phpMyAdmin**, and creating robust server environments using **Proxmox** Virtual Machines.
                                                </p>
                                                <ul class="list-none space-y-2 text-text-default">
                                                    <?php
                                                    // Fetch category ID for Period 2 DB
                                                    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
                                                    $stmt->execute(['Period 2: Database Management (MySQL, SQL, Proxmox)']);
                                                    $cat = $stmt->fetch();
                                                    $period2_id = $cat ? $cat['id'] : null;
                                                    $period2_files = $period2_id ? getFiles($pdo, $period2_id, $user_role, $user_id) : [];
                                                    if (empty($period2_files)): ?>
                                                    <li class="p-2 text-text-muted italic">No files uploaded yet.</li>
                                                    <?php else: ?>
                                                    <?php foreach ($period2_files as $file): ?>
                                                    <li class="p-2 border-b border-border-color flex justify-between items-center hover:bg-secondary rounded-sm transition-colors duration-300">
                                                        <span class="font-medium"><?php echo htmlspecialchars($file['title']); ?></span> 
                                                        <span class="text-sm text-text-muted hidden md:block"><?php echo htmlspecialchars($file['description']); ?></span>
                                                        <a href="<?php echo htmlspecialchars($file['file_path']); ?>" target="_blank" class="text-accent hover:text-accent-dark text-sm font-medium" data-filetype="<?php echo pathinfo($file['file_path'], PATHINFO_EXTENSION); ?>">(View Artifact)</a>
                                        ...
                                        <!-- Period 2: Professional Skills & Documentation (Separate Module) -->
                                        <div class="bg-primary p-4 rounded-lg shadow-inner border border-accent/70 transition-colors duration-300">
                                            ...
                                            <div id="period-2-prof-skills" class="period-content hidden mt-3 space-y-2">
                                                <p class="text-sm text-text-muted mb-2 font-semibold border-b border-border-color pb-2">
                                                    Key non-technical documentation showcasing reporting and risk management skills for the database management module.
                                                </p>
                                                <ul class="list-none space-y-2 text-text-default pt-2">
                                                    <?php
                                                    // Fetch category ID for Period 2 Prof Skills
                                                    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
                                                    $stmt->execute(['Period 2 Professional Skills & Documentation']);
                                                    $cat = $stmt->fetch();
                                                    $period2_prof_id = $cat ? $cat['id'] : null;
                                                    $period2_prof_files = $period2_prof_id ? getFiles($pdo, $period2_prof_id, $user_role, $user_id) : [];
                                                    if (empty($period2_prof_files)): ?>
                                                    <li class="p-2 text-text-muted italic">No files uploaded yet.</li>
                                                    <?php else: ?>
                                                    <?php foreach ($period2_prof_files as $file): ?>
                                                    <li class="p-2 border-b border-border-color flex justify-between items-center hover:bg-secondary rounded-sm transition-colors duration-300">
                                                        <span class="font-medium"><?php echo htmlspecialchars($file['title']); ?></span> 
                                                        <span class="text-sm text-text-muted hidden md:block"><?php echo htmlspecialchars($file['description']); ?></span>
                                                        <a href="<?php echo htmlspecialchars($file['file_path']); ?>" target="_blank" class="text-accent hover:text-accent-dark text-sm font-medium" data-filetype="<?php echo pathinfo($file['file_path'], PATHINFO_EXTENSION); ?>">(View Artifact)</a>
                                ...
                                <!-- Other Codes & Tools Section (Remains at Year 1 level, outside semesters) -->
                                <div class="bg-primary p-4 rounded-lg shadow-inner border border-border-color transition-colors duration-300">
        ...
        <section id="contact" class="py-20 px-4 sm:px-6 lg:px-8 bg-primary border-t border-accent/20 transition-colors duration-300">
            <div class="container mx-auto max-w-3xl">
                ...
                <!-- Simple Contact Form (Non-functional without a backend) -->
